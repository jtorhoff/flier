...
with open("MTProtoSchema.ts", "w") as mtproto:
    ...
    mtproto.write("\n")
    # mtproto.write("\n".join(protocol_template.render(type="__MTPROTO_" + t)
    #     for t in aggregate_types.values()))
    # mtproto.write("\n\n")
    mtproto.write("export namespace MTProto {")
    # mtproto.write("\n")
    # mtproto.write(indent("// MARK: Protocol aliases"))
    # mtproto.write("\n")
    # mtproto.write("\n".join(indent("typealias {0} = __MTPROTO_{0}".format(t)) for t in aggregate_types.values()))
    # mtproto.write("\n\n")
    # mtproto.write(indent("// MARK: Constructables"))
    mtproto.write("\n")
    mtproto.write("\n".join(indent(t) for t in ts_union_types))
    mtproto.write("\n\n")
    # mtproto.write(indent("// MARK: Objects"))
    # mtproto.write("\n")
    ...
    mtproto.write("\n")
    # mtproto.write(indent("// MARK: Functions"))
    # mtproto.write("\n")
    # mtproto.write("\n\n".join(ts_functions))
    # mtproto.write("\n")
